...
test('tag: allows a custom list of standard tags', async t => {
  const { error, output } = await execute(
    '--checks tag --standard-tags next latest',
    {
...
test('changelog: allows a custom changelog path', async t => {
  const { error, output } = await execute(
    '--checks changelog --changelog-path ../changelog/CHANGELOG.md',
    {
      npmVersion: '1.1.0',
...
test('branch: allows multiple release branches to be specified', async t => {
  const { error, output } = await execute(
    '--checks branch --branch-name dev something-else',
    {
      path: path.join(__dirname, '../test/bin/dev')
    }
  );
...
test('branch: ignores prerelease versions', async t => {
  const { error, output } = await execute(
    '--checks branch --branch-name nonexistant',
    {
      npmVersion: '1.0.0-beta.0'
    }
  );
...
test('branch: fails when publishing a standard version on the branch other than the provided one', async t => {
  const { error, output } = await execute(
    '--checks branch --branch-name other',
    {
      npmVersion: '1.0.0'
    }
  );
...
function validateOutput(
  ...
  output: string[],
  checks: { [K in 'tag' | 'changelog' | 'branch']?: boolean }
): void {
  const successText = {
...
async function execute(
...
): Promise<{ error: boolean; output: string[] }> {
  const options = opts || {};

  const env: NodeJS.ProcessEnv = {
    PATH: `${options.path || path.join(__dirname, '../test/bin/master')}:${
      process.env.PATH
    }`
  };
  ...
  try {
    output = (
      await util.promisify(child_process.exec)(`${executable} ${args}`, {
        encoding: 'utf8',
        cwd,
        env
      })
