...
export function parseArgs(): { version: string; command: string; args: string[]; } {
    const version = process.env.npm_package_version;
    if (!version) {
        throw new Error('Unable to fetch command context from npm. Make sure you\'re running pubcop in the prepublishOnly/prepublish npm script');
    }
