...
const argv = yargs.options({
  checks: {
    ...
    coerce: (checks: string[]) =>
      checks.some(check => check === 'all')
        ? ['tag', 'branch', 'changelog']
        : checks,
    description:
      "The space-separatd list of validations to perform. You may also specify 'all' instead to enable all validations. All validations are on by default."
  },
  'standard-tags': {
    alias: 'st',
    array: true,
    ...
    default: ['latest'],
    description:
      "Tags that are acceptable to use when publishing a standard release. Useful for projects that maintain a 'next' tag, for instance. Only used when tag validation is enabled."
  },
  'branch-name': {
    alias: 'bn',
    array: true,
    ...
    default: ['master'],
    description:
      'The branch(es) associated with a standard release. Publishes from other branches will be rejected. Only used when branch validation is enabled.'
  },
  'changelog-path': {
    alias: 'cp',
    string: true,
    ...
    description:
      'The location of the changelog file to verify. Only used when changelog validation is enabled.'
  }
}).argv;
...
async function main() {
  ...
  if (command !== 'publish') {
    throw new Error(
      `Cannot run pubcop for command '${command}'. Make sure you're running pubcop in the prepublishOnly/prepublish npm script`
    );
  }
  ...
  if (!dir) {
    throw new Error(
      "Unable to find a package.json for the current path. Make sure you're running pubcop in the prepublishOnly/prepublish npm script"
    );
  }
  ...
  if (checks.includes('tag')) {
    results.push(
      await check('Tag', checkTag(version, tag, argv['standard-tags']))
    );
  }

  if (checks.includes('branch')) {
    results.push(
      await check('Git Branch', checkBranch(version, argv['branch-name']))
    );
  }
  ...
  if (checks.includes('changelog')) {
    results.push(
      await check(
        'Changelog',
        checkChangelog(version, dir, argv['changelog-path'])
      )
    );
  }
...
async function check(
  name: string,
  operation: (string | void) | Promise<string | void>
): Promise<boolean> {
  try {
